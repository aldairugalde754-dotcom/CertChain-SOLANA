...
//#region node_modules/@metaplex-foundation/digital-asset-standard-api/dist/src/errors.js
var require_errors = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.DasApiError = void 0;
	var DasApiError = class extends Error {
		constructor() {
			super(...arguments);
...
//#endregion
//#region node_modules/@metaplex-foundation/digital-asset-standard-api/dist/src/decorator.js
var require_decorator = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.createDasApiDecorator = void 0;
	var errors_1 = require_errors();
	function cleanInput(obj) {
		return Object.fromEntries(Object.entries(obj).filter(([_, value]) => value !== null && value !== void 0 && (typeof value !== "object" || value && Object.keys(value).length > 0)));
	}
	var createDasApiDecorator = (rpc) => {
		const validatePagination = (page, before, after) => {
			if (typeof page === "number" && (before || after)) throw new errors_1.DasApiError("Pagination Error. Please use either page or before/after, but not both.");
		};
		return {
			...rpc,
			getAsset: async (input) => {
				const assetId = typeof input === "object" && "assetId" in input ? input.assetId : input;
				const displayOptions = typeof input === "object" && "displayOptions" in input ? input.displayOptions : {};
				const asset = await rpc.call("getAsset", {
					id: assetId,
					options: displayOptions
				});
				if (!asset) throw new errors_1.DasApiError(`Asset not found: ${assetId}`);
				return asset;
			},
			getAssets: async (input) => {
				...
				if (!assets) throw new errors_1.DasApiError(`No assets found: ${assetIds}`);
				return assets;
			},
			getAssetProof: async (assetId) => {
				const proof = await rpc.call("getAssetProof", { id: assetId });
				if (!proof) throw new errors_1.DasApiError(`No proof found for asset: ${assetId}`);
				return proof;
			},
			getAssetProofs: async (assetIds) => {
				const proofs = await rpc.call("getAssetProofs", { ids: assetIds });
				if (!proofs) throw new errors_1.DasApiError(`No proofs found for assets: ${assetIds}`);
				return proofs;
			},
			getAssetsByAuthority: async (input) => {
				...
				if (!assetList) throw new errors_1.DasApiError(`No assets found for authority: ${input.authority}`);
				return assetList;
			},
			getAssetsByCreator: async (input) => {
				...
				if (!assetList) throw new errors_1.DasApiError(`No assets found for creator: ${input.creator}`);
				return assetList;
			},
			getAssetsByGroup: async (input) => {
				...
				if (!assetList) throw new errors_1.DasApiError(`No assets found for group: ${input.groupKey} => ${input.groupValue}`);
				return assetList;
			},
			getAssetsByOwner: async (input) => {
				...
				if (!assetList) throw new errors_1.DasApiError(`No assets found for owner: ${input.owner}`);
				return assetList;
			},
			searchAssets: async (input) => {
				...
				if (!assetList) throw new errors_1.DasApiError("No assets found for the given search criteria");
				return assetList;
			},
			getAssetSignatures: async (input) => {
				...
				if (!signatures) {
					const identifier = "assetId" in input ? `asset: ${input.assetId}` : `tree: ${input.tree}, leaf_index: ${input.leaf_index}`;
...
//#endregion
//#region node_modules/@metaplex-foundation/digital-asset-standard-api/dist/src/plugin.js
var require_plugin = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.dasApi = void 0;
	var decorator_1 = require_decorator();
	var dasApi = () => ({ install(umi) {
		umi.rpc = (0, decorator_1.createDasApiDecorator)(umi.rpc);
	} });
	exports.dasApi = dasApi;
}));
//#endregion
//#region node_modules/@metaplex-foundation/digital-asset-standard-api/dist/src/types.js
var require_types = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
}));
//#endregion
//#region node_modules/@metaplex-foundation/digital-asset-standard-api/dist/src/index.js
